...
def json_dump_model_aggregated_profile(
    output_obj, model, module_depth=-1, top_modules=1
):
...
def json_dump_model_profile(
    ...
    if profiler.ds_engine:
        total_model_nonexpert_params = (
            profiler.model.__params__ * profiler.ds_engine.mp_world_size
        )
        if profiler.ds_engine.has_moe_layers:
            expert_tensor_parallelism = profiler.is_expert_tensor_parallelism_enabled()
            total_model_expert_params = profiler.model.__model_expert_params__ * (
                profiler.ds_engine.mp_world_size if expert_tensor_parallelism else 1
            )
    ...
    if profiler.ds_engine:
        json_add(
            psas_output, "world size", "{:<8}".format(profiler.ds_engine.world_size)
        )
        json_add(
            psas_output,
            "data parallel size",
            "{:<8}".format(profiler.ds_engine.dp_world_size),
        )
        json_add(
            psas_output,
            "model parallel size",
            "{:<8}".format(profiler.ds_engine.mp_world_size),
        )
        json_add(
            psas_output,
            "batch size per GPU",
            "{:<8}".format(profiler.ds_engine.train_micro_batch_size_per_gpu()),
        )
        if profiler.ds_engine.has_moe_layers:
            json_add(
                psas_output,
                "expert tensor parallelism enabled",
                "{:<8}".format(expert_tensor_parallelism),
            )
    ...
    if total_model_expert_params > 0:
        ...
        json_add(
            ...
            "{:<8}".format(params_to_string(total_model_nonexpert_params)),
        )
        json_add(
            psas_output,
            "expert params of model",
            "{:<8}".format(params_to_string(total_model_expert_params)),
        )
    else:
        json_add(
            psas_output, "params of model", "{:<8}".format(total_model_nonexpert_params)
        )

    json_add(
        psas_output, "fwd MACs per GPU", "{:<8}".format(macs_to_string(total_macs))
    )
    json_add(
        psas_output, "fwd flops per GPU", "{:<8}".format(num_to_string(total_flops))
    )
    json_add(
        psas_output,
        "fwd flops of model",
        "{:<8}".format(
            num_to_string(
                total_flops
                * ((profiler.ds_engine.mp_world_size) if profiler.ds_engine else 1)
            )
        ),
    )
    ...
    json_add(
        psas_output, "fwd latency", "{:<8}".format(duration_to_string(fwd_latency))
    )
    json_add(
        psas_output,
        "fwd FLOPS per GPU",
        "{:<8}".format(flops_to_string(total_flops / fwd_latency)),
    )
    ...
    if profiler.ds_engine and profiler.ds_engine.wall_clock_breakdown():
        ...
        json_add(
            psas_output, "bwd latency", "{:<8}".format(duration_to_string(bwd_latency))
        )
        json_add(
            psas_output,
            "bwd FLOPS per GPU",
            "{:<8}".format(
                flops_to_string(flops_to_string(bwd_factor * total_flops / bwd_latency))
            ),
        )
        json_add(
            psas_output,
            "fwd+bwd FLOPS per GPU",
            "{:<8}".format(
                flops_to_string(
        ...
        json_add(
            psas_output,
            "step latency",
            "{:<8}".format(duration_to_string(step_latency)),
        )
        ...
        json_add(
            psas_output,
            "iter latency",
            "{:<8}".format(duration_to_string(iter_latency)),
        )
        json_add(
            psas_output,
            "FLOPS per GPU",
        ...
        samples_per_iter = (
            profiler.ds_engine.train_micro_batch_size_per_gpu()
            * profiler.ds_engine.world_size
        )
        json_add(
            psas_output,
            "samples/second",
            "{:<8.2f}".format(samples_per_iter / iter_latency),
        )
    ...
    json_dump_model_aggregated_profile(
        appg_output, profiler.model, module_depth=module_depth, top_modules=top_modules
    )
...
def get_model_inference_profile(
    model,
    input_shape=None,
    args=[],
    kwargs={},
    ...
    def register_io_shape_hook(module, ignore_list):
        ...
        def io_shape_hook(module, input, output):
            ...
            if isinstance(output, (list, tuple)):
                module.__output_shape__ = [[-1] + list(o.size())[1:] for o in output]
            elif isinstance(output, dict):
                module.__output_shape__ = [
                    [-1] + list(v.size())[1:] if hasattr(v, "size") else list()
        ...
        if (
            ...
            module.__io_shape_hook_handle__ = module.register_forward_hook(
                io_shape_hook
            )
