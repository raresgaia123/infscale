...
def launch(world_size, split_size, partitions, shards, devices, pre_trained, logging):
    ...
    for i, node in enumerate(node_list):
        torchrun_cmd = (
            f'torchrun --nnodes=2 --node-rank={i + 1} --nproc-per-node=3 --rdzv-id=819 --rdzv-backend=c10d --rdzv-endpoint={host}:{host_port} resnet50/resnet50_multinode_inference.py "{args[0]}" "{args[1]}" "{args[2]}" '
            + " ".join(args[3:])
        )
        cmd = (
            f"ssh changwu@{node} "
            + '"'
            + "source .profile; cd LLM-Inference;"
            + torchrun_cmd
            + '"'
        )
    ...
    torchrun_cmd = (
        f'torchrun --nnodes=2 --node-rank=0 --nproc-per-node=3 --rdzv-id=819 --rdzv-backend=c10d --rdzv-endpoint={host}:{host_port} resnet50/resnet50_multinode_inference.py "{args[0]}" "{args[1]}" "{args[2]}" '
        + " ".join(args[3:])
    )
